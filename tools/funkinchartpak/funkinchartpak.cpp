...
{
    return (uint64_t)std::floor(pos / crochet + 0.5);
}
...
int main(int argc, char *argv[])
{
    ...
    for (auto &i : song_info["notes"]) //Iterate through sections
    {
        ...
        for (auto &j : i["sectionNotes"])
        {
            ...
            new_note.type = (uint16_t)j[1] % max_keys;
    ...
    Section dum_section;
    dum_section.end = 0xFFFFFFFFFFFFFFFF; // Changed to use a larger value
    dum_section.flag = sections[sections.size() - 1].flag;
    sections.push_back(dum_section);

    Note dum_note;
    dum_note.pos = 0xFFFFFFFFFFFFFFFF; // Changed to use a larger value
