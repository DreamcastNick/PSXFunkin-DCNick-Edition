/*
 * funkinsmallchartpak
 * Packs Friday Night Funkin' json formatted charts into a compact binary file for PSX
*/

#include <iostream>
#include <fstream>
#include <vector>
#include <cstdint>
#include <algorithm>
#include <unordered_set>
#include <string>

#include "../funkinchartpak/json.hpp"
using json = nlohmann::json;

#define SECTION_FLAG_OPPFOCUS (1ULL << 15)
#define SECTION_FLAG_BPM_MASK 0x7FFF

struct Section
{
    uint16_t end;
    uint16_t flag = 0;
};

#define NOTE_FLAG_SUSTAIN (1 << 5)
#define NOTE_FLAG_SUSTAIN_END (1 << 6)
#define NOTE_FLAG_ALT_ANIM (1 << 7)
#define NOTE_FLAG_MINE (1 << 8)
#define NOTE_FLAG_DANGER (1 << 9)
#define NOTE_FLAG_STATIC (1 << 10)
#define NOTE_FLAG_PHANTOM (1 << 11)
#define NOTE_FLAG_POLICE (1 << 12)
#define NOTE_FLAG_MAGIC (1 << 13)
#define NOTE_FLAG_HIT (1 << 14)

struct Note
{
    uint16_t pos;
    uint16_t type;
    uint16_t is_opponent;
};

uint16_t ChartKey(json &j)
{
    switch (j.value("mania", 0))
    {
    case 0: return 4;
    case 1: return 6;
    case 2: return 7;
    case 3: return 9;
    case 4: return 5;
    }
    return 4;
}

uint64_t PosRound(double pos, double crochet)
{
    return static_cast<uint64_t>(std::floor(pos / crochet + 0.5));
}

double ReadJsonNumber(const json &obj, const char *key, double fallback)
{
    auto it = obj.find(key);
    if (it == obj.end() || it->is_null()) return fallback;
    if (it->is_number()) return it->get<double>();
    if (it->is_string())
    {
        try { return std::stod(it->get<std::string>()); }
        catch (...) { return fallback; }
    }
    return fallback;
}

void WriteWord(std::ostream &out, uint16_t word)
{
    out.put(word >> 0);
    out.put(word >> 8);
}

int main(int argc, char *argv[])
{
    if (argc < 2)
    {
        std::cout << "usage: funkinsmallchartpak in_json" << std::endl;
        return 0;
    }

    std::ifstream i(argv[1]);
    if (!i.is_open())
    {
        std::cout << "Failed to open " << argv[1] << std::endl;
        return 1;
    }

    json j;
    i >> j;
    auto song_info = j["song"];

    double bpm = ReadJsonNumber(song_info, "bpm", 100.0);
    if (bpm <= 0.0) bpm = 100.0;
    double crochet = (60.0 / bpm) * 1000.0;
    double step_crochet = crochet / 4;

    uint16_t keys = ChartKey(song_info);
    uint16_t max_keys = keys * 2;

    uint64_t milli_base = 0;
    uint64_t step_base = 0;

    std::vector<Section> sections;
    std::vector<Note> notes;

    uint64_t section_end = 0;
    for (auto &i : song_info["notes"])
    {
        bool is_opponent = i["mustHitSection"] != true;

        Section new_section;
        if (i["changeBPM"] == true)
        {
            milli_base += step_crochet * (section_end - step_base);
            step_base = section_end;

            bpm = ReadJsonNumber(i, "bpm", bpm);
            if (bpm <= 0.0) bpm = 100.0;
            crochet = (60.0 / bpm) * 1000.0;
            step_crochet = crochet / 4;
        }

        new_section.end = static_cast<uint16_t>((section_end += 16) * 12);
        new_section.flag = PosRound(bpm, 1.0 / 24.0) & SECTION_FLAG_BPM_MASK;
        bool is_alt = i["altAnim"] == true;
        if (is_opponent) new_section.flag |= SECTION_FLAG_OPPFOCUS;
        sections.push_back(new_section);

        for (auto &j : i["sectionNotes"])
        {
            Note new_note;
            int sustain = static_cast<int>(PosRound(j[2], step_crochet)) - 1;
            uint64_t note_pos = (step_base * 12) + PosRound(((uint64_t)j[0] - milli_base) * 12.0, step_crochet);
            new_note.pos = static_cast<uint16_t>(note_pos);
            new_note.type = static_cast<uint16_t>(j[1]) % max_keys;
            new_note.is_opponent = 0;

            if ((!is_opponent && new_note.type >= keys) || (is_opponent && new_note.type < keys))
                new_note.is_opponent = 1;

            if (is_opponent)
                new_note.type = (new_note.type + keys) % max_keys;

            if (j[3] == "Alt Animation")
                new_note.type |= NOTE_FLAG_ALT_ANIM;
            else if ((new_note.is_opponent) && is_alt)
                new_note.type |= NOTE_FLAG_ALT_ANIM;

            if (sustain >= 0)
                new_note.type |= NOTE_FLAG_SUSTAIN_END;

            if (j[3] == "Hurt Note") new_note.type |= NOTE_FLAG_MINE;
            if (j[3] == "HD Note") new_note.type |= NOTE_FLAG_DANGER;
            if (j[3] == "Static Note") new_note.type |= NOTE_FLAG_STATIC;
            if (j[3] == "Phantom Note") new_note.type |= NOTE_FLAG_PHANTOM;
            if (j[3] == "police") new_note.type |= NOTE_FLAG_POLICE;
            if (j[3] == "magic") new_note.type |= NOTE_FLAG_MAGIC;

            notes.push_back(new_note);

            for (int k = 0; k <= sustain; k++)
            {
                Note sus_note;
                sus_note.pos = static_cast<uint16_t>(new_note.pos + ((k + 1) * 12));
                sus_note.type = new_note.type | NOTE_FLAG_SUSTAIN;
                sus_note.is_opponent = new_note.is_opponent;

                if (k != sustain)
                    sus_note.type &= ~NOTE_FLAG_SUSTAIN_END;
                notes.push_back(sus_note);
            }
        }
    }

    std::sort(notes.begin(), notes.end(), [](Note a, Note b) {
        if (a.pos == b.pos)
            return (b.type & NOTE_FLAG_SUSTAIN) && !(a.type & NOTE_FLAG_SUSTAIN);
        return a.pos < b.pos;
    });

    Section dum_section;
    dum_section.end = 0xFFFF;
    dum_section.flag = sections[sections.size() - 1].flag;
    sections.push_back(dum_section);

    Note dum_note;
    dum_note.pos = 0xFFFF;
    dum_note.type = NOTE_FLAG_HIT;
    dum_note.is_opponent = 0;
    notes.push_back(dum_note);

    std::ofstream out(std::string(argv[1]) + ".chs", std::ostream::binary);
    if (!out.is_open())
    {
        std::cout << "Failed to open " << argv[1] << ".chs" << std::endl;
        return 1;
    }

    const uint16_t note_offset = static_cast<uint16_t>(4 + (sections.size() << 2));
    WriteWord(out, keys);
    WriteWord(out, note_offset);

    for (auto &i : sections)
    {
        WriteWord(out, i.end);
        WriteWord(out, i.flag);
    }

    for (auto &i : notes)
    {
        WriteWord(out, i.pos);
        WriteWord(out, i.type);
        WriteWord(out, i.is_opponent);
    }

    return 0;
}
